document.addEventListener('DOMContentLoaded', () => {
                ...
                <div class="shelf-content">
                    ${shelf.items.map(item => `
                        <div class="item-editor-box" data-shelf-id="${shelf.id}" data-item-id="${item.id}">
                            <div class="relative w-full text-center">${item.type === 'container' ? '&#128451;' : ''}</div>
                            <img src="${item.img || 'https://placehold.co/60x60/d1d5db/4b5563?text=Item'}" alt="${item.name}">
                            <span class="item-name">${item.name || 'New Item'}</span>
                        </div>
                    `).join('')}
                    <button class="add-item-btn w-16 h-full flex items-center justify-center text-3xl bg-gray-200 hover:bg-gray-300 text-gray-500 rounded-md" data-shelf-id="${shelf.id}">+</button>
                </div>
            `;
    ...
    function openItemEditor(shelfId, itemId, isSubItem = false, parentItemId = null) {
        ...
        if (!editorModal.overlay || !item) return;
        ...
        editorModal.typeSelectorContainer.classList.toggle('hidden', isSubItem);
    ...
    function closeItemEditor() {
        ...
        if (editorModal.fileInput) editorModal.fileInput.value = '';
        tempImageSrc = null;
    }
    ...
    function handleImageUpload(e) {
        ...
        if (!file || !editorModal.overlay) return;
        editorModal.imagePreview.src = URL.createObjectURL(file);
    ...
    addSafeEventListener(editorUI.doneBtn, 'click', () => {
        const locker = findLockerById(currentlyEditing.lockerId);
        if (locker) {
            locker.name = editorUI.lockerName.value;
            saveData().then(() => {
    ...
    delegateEvent('locker-editor-shelves', 'click', '.add-item-btn', (e, btn) => addItemToShelf(parseInt(btn.dataset.shelfId)));
